...
// ===================================================================
// TerrainManager — Malha única para o Builder
// Usa getHeight/getMoisture/getBiome definidos acima
// ===================================================================
...
export class TerrainManager {
    constructor(scene) {
        ...
        this.segments = 128; // Resolução padrão — alterável via setResolution()
    }

    // 1.3 — Resolução configurável
    setResolution(segments) {
        const valid = [64, 128, 256];
        this.segments = valid.includes(segments) ? segments : 128;
        console.log(`🔧 TerrainManager: resolução definida para ${this.segments}x${this.segments}`);
    }
    ...
    generate(seed) {
        ...
        const SIZE = 256;
        const SEGMENTS = this.segments; // Usa resolução configurada
        ...
        for (let i = 0; i < positions.count; i++) {
            ...
            const h = getHeight(x, z, seed);
            positions.setY(i, h);

            const moisture = getMoisture(x, z, seed);
            const biome = getBiome(h, moisture, seed);
            const hex = BIOME_COLORS[biome] ?? 0x3a7c3a;

            this._color.setHex(hex);
            // Micro-variação por vértice para evitar look plano
            const v = (Math.random() - 0.5) * 0.04;
            colors.push(
                Math.max(0, Math.min(1, this._color.r + v)),
                Math.max(0, Math.min(1, this._color.g + v)),
                Math.max(0, Math.min(1, this._color.b + v))
            );
        }
        ...
        console.log(`✅ TerrainManager: malha gerada (seed=${seed}, ${SEGMENTS}x${SEGMENTS} segmentos)`);
    }
}
